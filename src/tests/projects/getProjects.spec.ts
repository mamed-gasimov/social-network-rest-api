...
describe('GET projects by pageSize and page', () => {
  it('should return list of projects, 200 status code and "X-total-count" response header', async () => {
    const responsePayload = {
      rows: [{ ...mockCreateProjectPayload, id: 1 } as unknown as projectModel],
      count: 1,
    };
    jest.spyOn(userModel, 'findOne').mockResolvedValueOnce({ ...mockResponseUser, role: UserRole.Admin } as userModel);
    jest.spyOn(projectModel, 'findAndCountAll').mockResolvedValueOnce(responsePayload);

    const response = await supertest(app)
      .get(`${ROUTES.project.main}?pageSize=1&page=1`)
  ...
  it('should return ""pageSize" is required" and 400 status code', async () => {
    ...
    const response = await supertest(app).get(`${ROUTES.project.main}?page=1`).send();
  ...
  it('should return ""page" is required" and 400 status code', async () => {
    ...
    const response = await supertest(app).get(`${ROUTES.project.main}?pageSize=1`).send();
  ...
  it('should return an empty object and 401 status code when not authorized', async () => {
    ...
    const response = await supertest(app).get(`${ROUTES.project.main}?pageSize=1&page=1`).send();
